...
export async function flushPendingBallots(electionId, session = null) {
  const pending = await BallotStaging.find(electionId ? { electionId } : {}, null, { session }).lean();
  ...
  for (const [electionIdKey, items] of byElection) {
    const last = await Ballot.findOne({ electionId: electionIdKey }).session(session).sort({ _id: -1 }).lean();
    ...
    await Ballot.insertMany(docs, { session });
    await BallotStaging.deleteMany({ _id: { $in: items.map((p) => p._id) } }, { session });
    flushed += docs.length;
  }
